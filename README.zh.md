...
- 🌐 **双语界面**：中文 / English 一键切换（请求 `lang` 字段控制解卦语言）
- 🖥️ **响应式 Web 界面**：简洁响应式，手机电脑都能用
...
解卦时若问题命中知识库内容，会自动注入相关段落作为参考。不设置则跳过，纯 LLM 解卦。（RAG 仅支持中文知识库；英文模式使用内置英文卦辞。）
...
`lang` 字段控制解卦语言（`zh` 或 `en`，默认 `zh`），Web 界面有切换按钮。返回本卦、动爻、变卦、LLM 解卦全文。
...
│   ├── iching_en.py      # 英文卦辞（judgment & image，自有译文）
│   ├── static/           # Web 前端
│   └── templates/
├── requirements.txt
└── .env.example
...
英文卦辞（judgment & image）为本项目自译，参考公版 Wilhelm 译本（1923）的诠释方向，非逐字原文（Wilhelm 英文版 1950 出版，尚未公版）。爻辞不内置英译，英文模式下由 LLM 参考中文原文进行解读。
